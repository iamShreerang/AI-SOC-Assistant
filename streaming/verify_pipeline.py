...
def main() -> None:
    ...
    # 8. Threat detection rules
    section("8. Threat Detection Rules")
    from streaming.spark.threat_rules import ThreatRuleEngine

    engine = ThreatRuleEngine()

    single_attack = engine.evaluate([
        {"srcip": "10.0.0.1", "dstip": "10.0.0.2", "label": 1, "confidence": 0.95,
         "sbytes": 500, "dbytes": 200, "dsport": 443},
    ])
    all_passed &= check(
        "Single high-confidence attack -> critical alert",
        len(single_attack) == 1 and single_attack[0]["severity"] == "critical",
        f"got {single_attack}",
    )

    repeated = engine.evaluate([
        {"srcip": "10.0.0.9", "dstip": "10.0.0.2", "label": 1, "confidence": 0.6,
         "sbytes": 100, "dbytes": 50, "dsport": 22}
        for _ in range(3)
    ])
    all_passed &= check(
        "3 attacks from same IP -> repeated-attack escalation",
        any("Repeated attack pattern" in a["title"] for a in repeated),
        f"got {[a['title'] for a in repeated]}",
    )

    large_transfer = engine.evaluate([
        {"srcip": "10.0.0.5", "dstip": "10.0.0.2", "label": 0, "confidence": 0.1,
         "sbytes": 2_000_000, "dbytes": 0, "dsport": 80},
    ])
    all_passed &= check(
        "Oversized flow -> large-data-transfer alert",
        any("Large data transfer" in a["title"] for a in large_transfer),
        f"got {[a['title'] for a in large_transfer]}",
    )

    port_scan = engine.evaluate([
        {"srcip": "10.0.0.7", "dstip": "10.0.0.2", "label": 0, "confidence": 0.1,
         "sbytes": 60, "dbytes": 0, "dsport": p}
        for p in range(1, 9)
    ])
    all_passed &= check(
        "7+ distinct dst ports from one IP -> port-scan alert",
        any("Port scan pattern" in a["title"] for a in port_scan),
        f"got {[a['title'] for a in port_scan]}",
    )
