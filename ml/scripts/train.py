"""
Train CNN-LSTM on UNSW-NB15 and save artifacts to ml/saved_models/.
...
def load_data() -> pd.DataFrame:
    """Combine train + test CSVs, then do a proper stratified split."""
    train = pd.read_csv(DATA_DIR / "UNSW_NB15_training-set.csv")
    test = pd.read_csv(DATA_DIR / "UNSW_NB15_testing-set.csv")
    combined = pd.concat([train, test], ignore_index=True)
    logger.info("Combined dataset: %d rows | attack=%.1f%%",
                len(combined), combined["label"].mean() * 100)
    return combined
